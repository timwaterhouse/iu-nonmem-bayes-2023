...
%  \textcolor{mrgteal}{Day 1 Start: 9:00am}
  ...
  % \item Example 3: Full Bayes popPK with selective use of informative
...
\begin{frame}
  ...
  \begin{itemize}
  \item<1-> Full Bayesian analysis requires:
    \begin{itemize}
    ...
    \end{itemize}
  \item<2-> For most realistic problems, those are very computationally
    demanding tasks.
  \item<3-> Increases in computation speed and development of new
...
\begin{itemize}
\item<1-> Involves random draws from approximate distributions and then correcting those draws to better approximate the joint posterior.
\item<2-> The samples are drawn sequentially so that each draw depends on the previous one, thus forming a Markov chain.
\item<3-> Eventually the Markov chain converges (in distribution) to a stationary distribution that is the joint posterior distribution.
\item<4-> Algorithms for MCMC include:
\begin{itemize}
\item Metropolis-Hastings algorithm
\item Gibbs sampling
\item Hamiltonian Monte Carlo (HMC) simulation
\end{itemize}
\item<5-> MCMC samples are serially correlated:
\begin{itemize}
\item Inferences based on MCMC require more samples than would be required for independent samples
\end{itemize}
\item<6-> Practical consequences:
\begin{itemize}
...
% \vskip 2em
%   \begin{enumerate}
%   \item Generate $y_t \sim q\left(y|x^{(t)}\right)$.
%   \item $x^{(t+1)} = \left\{\begin{array}{ll}
%       y_t, & \text{with probability } \rho\left(x^{(t)}, y_t\right) \\
%       x^{(t)}, & \text{with probability } 1 - \rho\left(x^{(t)}, y_t\right) 
%     \end{array} \right.$ \\
%   where $\displaystyle \rho\left(x, y\right) =
...
% \section{Example 1: Example illustrating Bayesian data analysis workflow}

% \begin{frame}
%   \frametitle{Example 1: Example illustrating Bayesian data analysis workflow}
...
% \section{Example 2: MAP popPK with selective use of informative priors}

% \begin{frame}
%   \frametitle{Example 2: MAP popPK with selective use of informative
%     priors for nuisance parameters: pediatric atorvastatin}
...
% \section[Example 3: Full Bayes popPK]{Example 3: Full Bayes popPK with selective use of
%   informative priors}

% \begin{frame}
%   \frametitle{Example 3: Full Bayes popPK with selective use of
%     informative priors for nuisance parameters: pediatric
%     atorvastatin}
  
%   \begin{itemize}
%   \item Let's repeat the analysis of the pediatric atorvastatin data
%     from Example 1 using METHOD = NUTS.
...
\end{itemize}
    \column{0.5\textwidth}
    \fbox{\includegraphics[width=\textwidth]{graphics/npde-equations.pdf}}
    \cite{Comets2008-lz}
  \end{columns}
...
%   \end{scriptsize}
%   \item Friberg-Karlsson semi-mechanistic model for
% drug-induced myelosuppression \cite{Friberg2002-tt,Friberg2003-ki,Latz2006-dk,Troconiz2006-nq,Kathman2007-ne,3537}
%   \end{itemize}
% \vspace{-6pt}
% \begin{center}
% \includegraphics[width=0.75\textwidth]{graphics/neutrophilModel.jpg}\\
...
% \bibliography{./custom,$HOME/pp-references/MetrumRGalpha}
\bibliography{$HOME/pp-references/MetrumRGalpha,./custom}
